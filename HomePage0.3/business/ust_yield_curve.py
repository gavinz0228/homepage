...
field_mappings = {
    'Id':'id', 'NEW_DATE':'date', 'BC_1MONTH':'1month','BC_2MONTH':'2month', 'BC_3MONTH':'3month',
...
def get_yield_curve_curr_month():
    ...
    return parse_xml(get(url), datetime(datetime.now().year, datetime.now().month, 1 ))
...
def get_yield_curve_bus_adj(date):
    data = parse_xml(get(construct_url(date.year, date.month, date.day)), date)
    curr_date = date
    day_adjusted = 0
    while len(data) == 0:
        ...
        data = parse_xml(get(construct_url(curr_date.year, curr_date.month, curr_date.day)), curr_date)
...
def get_yield_curve(year=None, month=None, day=None):
    ...
    return parse_xml(get(construct_url(year, month, day)), datetime(year, month, day) )

def get_yield_curve_by_dates(dates):
     return [ get_yield_curve_bus_adj(dt) for dt in dates]

def parse_xml(xml_str, date):
